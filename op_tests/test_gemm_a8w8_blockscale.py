...
def test_gemm(dtype, m, n, k, ck_preshuffle=True):
    ...
    err_c = checkAllclose(a, c, msg="{tag}")
    ret[f"{tag} us"] = avg_c
    ...
    msg = f"[perf] dim: {str(dim):<50} dtype: {dtype}, torch avg: {avg_a:<8.2f} us, ck avg: {avg_b:<8.2f} us, asm avg: {avg_c:<8.2f} us, uplift: {avg_b/avg_c -1:<5.1%}"
    failed = checkAllclose(a, c, msg="a,c: " + msg, rtol=1e-2, atol=0.01)
    ret["failed"] = failed
...
def run_asm(x, weight, x_scale, w_scale, dtype=dtypes.bf16, kernel_name=None):
    ...
    return aiter.gemm_a8w8_blockscale_bpreshuffleASM(x, weight, out, x_scale, w_scale)
...
if df["failed"].any():
    print("Failed cases:", df[df["failed"]], sep="\n")
